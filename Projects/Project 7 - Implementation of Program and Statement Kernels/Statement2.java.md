...
public class Statement2 extends StatementSecondary {
    ...
    private void createNewRep() {
        ...
        Sequence<Tree<StatementLabel>> sequence = this.rep.newSequenceOfTree();
        this.rep.assemble(new StatementLabel(StatementKernel.Kind.BLOCK), sequence);
    }
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        Statement2 localStatement = (Statement2) s;
        this.rep.addSubtree(pos, localStatement.rep);
        localStatement.createNewRep();
    }
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        Statement2 localStatement1 = (Statement2) s1;
        Statement2 localStatement2 = (Statement2) s2;

        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);

        Sequence<Tree<StatementLabel>> sequence = this.rep.newSequenceOfTree();
        sequence.add(0, localStatement1.rep);
        sequence.add(1, localStatement2.rep);

        this.rep.assemble(label, sequence);

        localStatement1.createNewRep();
        localStatement2.createNewRep();
    }
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> sequence = this.rep.newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(sequence);
        localStatement1.rep = sequence.remove(0);
        localStatement2.rep = sequence.remove(0);
        this.createNewRep();
        return label.condition;
    }
    ...
    public final void assembleWhile(Condition c, Statement s) {
        ...
        Sequence<Tree<Statement2.StatementLabel>> sequence = this.rep.newSequenceOfTree();
        sequence.add(0, localStatement.rep);
        this.rep.assemble(label, sequence);
        localStatement.createNewRep();
    }
    ...
    public final Condition disassembleWhile(Statement s) {
        ...
        StatementLabel label = this.rep.disassemble(sequence);
        localStatement.rep = sequence.remove(0);
        this.createNewRep();
    ...
    public final String disassembleCall() {
        ...
        StatementLabel label = this.rep.root();
        this.createNewRep();
